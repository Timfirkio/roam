...
public class RideTrackingService extends Service implements LocationListener {
  ...
  private RidePointStore pointStore;
  @Override public void onCreate() { super.onCreate(); manager=(LocationManager)getSystemService(LOCATION_SERVICE); pointStore=RidePointStore.get(this); if(Build.VERSION.SDK_INT>=26)((NotificationManager)getSystemService(NOTIFICATION_SERVICE)).createNotificationChannel(new NotificationChannel(CHANNEL,"Ride tracking",NotificationManager.IMPORTANCE_LOW)); }
  @Override public int onStartCommand(Intent intent,int flags,int id) {
    ...
    if(!alreadyActive) {
      pointStore.reset();
      preferences.edit().putBoolean("active",true).putLong("startedAt",System.currentTimeMillis()).apply();
    }
