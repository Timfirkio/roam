...
import { cachedExploredTotals, calculateExploredAreaTotals, exploredTotalsKey, latestExploredAreaTotals, previousExploredAreaTotals, rememberExploredAreaTotals } from './area-progress-calculation';
...
export function AreaCoverageCard({ record, discoveries, onUpdate, onExplored, parentAreaName, reserveParentArea = false, showActions = true, dataReady = true, syncReady = true, className, onClick }: { record: AreaRecord | null; discoveries: DiscoveredSegment[]; onUpdate: (record: AreaRecord) => void; onExplored: (areaId: string, totals: AreaTotals) => void; parentAreaName?: string; reserveParentArea?: boolean; showActions?: boolean; dataReady?: boolean; syncReady?: boolean; className?: string; onClick?: () => void }) {
  ...
  const totalLength = ready ? ready.lengthMeters : displayedTotals?.network.lengthMeters ?? null;
  const totalLabel = totalLength !== null ? distance(totalLength) : '—';
  ...
  return <Item variant="outline" className={`area-progress-card ${className ?? ''}`} role={onClick ? 'button' : undefined} tabIndex={onClick ? 0 : undefined} onClick={onClick} onKeyDown={event => { if (onClick && (event.key === 'Enter' || event.key === ' ')) { event.preventDefault(); onClick(); } }}>
    <ItemContent className="district-progress-content">
      ...
      <div className="district-progress-top" aria-busy={loading}><div className="district-progress-title"><ScrambleText text={displayName} /></div></div>
      ...
      <div className="district-progress-readouts" aria-busy={loading}><ItemDescription className="district-progress-description"><AnimatedProgressValue value={displayedTotals?.explored.lengthMeters ?? null} label={exploredLabel} className="area-progress-distance" /><span className="district-progress-distance-separator"> / </span><AnimatedProgressValue value={totalLength} label={totalLabel} className="area-progress-distance" /></ItemDescription><div className="district-progress-percent"><AnimatedPercentage white value={percentage !== null && !inconsistent ? percentage : null} /></div></div>
      ...
      {showActions && area && !ready && <ItemActions className="area-progress-actions"><Button variant="secondary" size="small" disabled={pending || requesting} onClick={event => { event.stopPropagation(); void calculate(); }}>{pending || requesting ? 'Calculating coverage' : job?.status === 'failed' ? 'Retry calculation' : 'Calculate coverage'}</Button></ItemActions>}
    </ItemContent>
  </Item>;
}

function useExploredAreaTotals(discoveries: DiscoveredSegment[], geometry: AreaRecord['area']['geometry'] | null, areaKey: string, enabled: boolean) {
  ...
  useEffect(() => {
    ...
    map.on('load', () => {
      ...
      if (import.meta.env.VITE_AREA_CATALOG !== 'false') {
        map.addSource(AREA_TILE_SOURCE, { type: 'vector', tiles: [areaTileUrlTemplate()], minzoom: 0, maxzoom: 22, promoteId: 'id' });
...
export function AreaProgressView({ location, discoveries }: { location: { lng: number; lat: number }; discoveries: DiscoveredSegment[] }) {
  ...
  return <section className="progress-map-view"><ProgressMap centre={centre} discoveries={discoveries} selected={selected} onSelectId={select} onHover={setHovered} /><LocationSearch onSelect={result => { setCentre([result.lng, result.lat]); select(result.id); }} />{hovered && <div className="progress-map-hover" role="status"><strong>{displayAreaName(hovered.name, hovered.adminLevel)}</strong><span>{hoveredPercentage === null ? '—' : `${hoveredPercentage.toFixed(1)}%`}</span></div>}{selected && <div className="progress-map-overlay"><AreaCoverageCard record={selected} discoveries={discoveries} onUpdate={updateRecord} onExplored={saveExplored} /></div>}{error && <div className="progress-map-message" role="alert">{error}</div>}</section>;
}
